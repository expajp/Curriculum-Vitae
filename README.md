...
#### 学んだこと
* チームの問題を早く拾い上げて対処するには、定期的にふりかえりを行って、出てきた課題をその場で議論するのが良いということ
* 常にチームの状況を把握するのには、簡単にモニタリングできる仕組みを作って、見るのを習慣にするのが良いということ
* コードベースの問題を拾い上げるには、気付いたことはアサインなしで起票できる仕組みが良いということ
...
* 社内で企画された新機能開発
  * システム内でバラバラに管理されているクライアント情報を統一的に管理する機能（3ヶ月）
    * 開発リーダーを担当（開発者2名、ユーザとなるビジネス側3名）
  * クライアントが自社技術を別のクライアントに提案する機能（4ヶ月）
    * 1名での開発
  
#### 学んだこと
* ソフトウェア設計の重要性
* 名付けをステークホルダー全員で検討することの重要性
* 設計を顧客にプレゼンする際、画面イメージに本質的なこと以外は書いてはいけないということ
...
#### 学んだこと
* 連絡を正確かつ迅速に取ることが信頼を築くということ
* 会社という組織は正しいルートで承認を得れば、なんでもできる場所であること
* 会社のブランディングは草の根でやり続けなければ実らないということ
