# 職務経歴書
2018年9月2日 更新

## 基本情報
|key|value|
|---|-----|
|Name|大河原 修 (Shu OGAWARA)|
|Email|expajp(at)gmail.com|
|Blog|[部屋の隅っこで書く技術ブログ](http://expajp-tech.hatenablog.com/)|
|GitHub|[expajp \(Shu OGAWARA\)](https://github.com/expajp)|
|Twitter|[@expajp](https://twitter.com/expajp)|
|Facebook|[大河原 修](https://www.facebook.com/expa.Shu)|
|Qiita|[expajp](https://qiita.com/expajp)|
|SlideShare(-2018/03)|[expajp](https://www.slideshare.net/expajp)|
|Speakerdeck(2018/04-)|[expajp](https://speakerdeck.com/expajp)|


## 主なスキル
### 現在、仕事で利用している
* Ruby/Ruby On Rails
* JavaScript(ES5, Coffee Script, jQuery)
* Bootstrap
* MySQL
* Git, Gitlab

### プロダクト作成に利用した経験がある
* VB/VBA, ASP
* SQL Server
* Google Apps Script
* FORTRAN
* C言語
* Java (Android)
* ZURB Foundation

### その他、現在勉強中である
* Vue.js

### 自然言語
* 日本語
  * 母語
* 英語
  * TOEIC公開テスト スコア715 (2015/08)
  * 海外での勤務経験はなし
  * 学生時代、短期間の交換留学プログラムに参加経験あり
    * [Brown\-ICERM\-Kobe Simulation Summer School \| Division of Applied Mathematics](https://www.brown.edu/academics/applied-mathematics/brown-icerm-kobe)


## 職務以外で開発したプロダクト
* 関西ラーメンbot for Android
  * 友人との共同開発
  * 2014年10月リリース（現在Google Playより削除済み、面接時に手元でデモ可能）
  * 現在地と最寄駅から最寄のラーメン屋を探し、レビューを閲覧するアプリ
* [計算基盤研究室 \- 神戸大学大学院システム情報学研究科](http://www.na.scitec.kobe-u.ac.jp/)
  * 所属していた研究室のWebサイト
  * ZURB Foundation を採用
  * デザイン、設計からリリースまですべて独力

## 資格
応用情報技術者を再取得したのは、プロジェクトマネージャ試験の午前I免除を兼ねて学部時代の知識を思い出すため。

|資格名|取得年月|
|---|-----|
|基本情報技術者|2012/05|
|応用情報技術者|2012/12, 2016/12|
|TOEIC 公開テスト Score.715|2015/08|
|プロジェクトマネージャ|2017/06|

## 強み
* 継続的な学習が習慣づいているため、職務に必要な技術へのキャッチアップはすぐ行えます
  * 2017年7月頃から基本的に毎日コードを書くようにしており、[GitHubのプロフィールページ](https://github.com/expajp)より確認できます
  * 主に[Project Euler](https://projecteuler.net/)をRubyで解いており、2018年9月現在96問解答済みです（参加者83万人中上位1.5%以内）
    * これにより、アルゴリズムの設計と実装が得意です
  * 趣味で学習を進める過程でビットコインの原著論文を英語で通読して社外発表しているため、**ブロックチェーンの基盤技術には十分な理解があります**
* エンジニア組織にマネジメントに強い関心を持っています
  * 「**個々人の能力の和よりも大きな力を発揮できるのが組織であり、少数の天才に頼るモデルより再現性が高い**」という考えによるものです
  * 上記の通り、IPAのプロジェクトマネージャ試験に合格しています
  * 「ピープルウェア」、「ザ・ファシリテーター」、「カイゼン・ジャーニー」など組織マネジメントに関わる本の他、「計画の科学」「マズロー心理学入門」などマネジメント周辺の知識が書かれた本を読んで勉強を進めています
    * 読んだ本は適宜読書メモにまとめています
* プレゼンテーションを得意としています
  * 学生時代に5年間、週2で塾講師として勤めていたことによるものです
  * 登壇歴は後述します


## 登壇歴
* [TokyuRuby会議12](https://tokyurubykaigi.github.io/tokyu12/)（2018/07）
  * [Project Eulerから生まれたワンライナーたち](http://expajp-tech.hatenablog.com/entry/2018/07/31/232737)
* [Y8 2017 spring in Shibuya](http://y8-2017-spring.hachiojipm.org/) (2017/05)
  * [PMをめざして1年が経った](https://www.slideshare.net/expajp/pm1-76397528)
* サポーターズColab.
  * [ブロックチェーンの基本の「き」](https://supporterzcolab.com/event/193/)(2017/10)
  * [LT型勉強会『ビットコイン』](https://supporterzcolab.com/event/268/)(2018/01)
  * [【ハンズオン】Project Eulerではじめる実装とアルゴリズム構築のトレーニング](https://supporterzcolab.com/event/387/)(2018/05)
  * [しくじり先生「新卒2年目の転職」](https://supporterzcolab.com/event/429/)(2018/06)
* [Otemachi.rb](https://enechange-meetup.connpass.com/)
  * ほぼ毎回LT参加


## 職務経歴
### 2017/11 - 現在: リンカーズ株式会社
職務: エンジニア（サーバサイドがメインだが、フロントエンドも行う）

#### RubyKaigi参加の取りまとめ
2018/02-06

ノベルティ作成およびブースの企画運営

* エンジニア採用のため、ノベルティの選定・作成・発注の取りまとめ
* ブース運営について、シフト作成・ノベルティの配布数管理等の取りまとめ

#### ビジネスマッチングプラットフォーム: Linkers の新機能開発
2017/01-06

発注側企業へ自社技術を提案できる新機能を実装

* 【担当フェーズ】詳細設計、プログラミング、単体テスト、結合テスト
* ER図, クラス図を用いた詳細設計
* Ruby On Railsを用いた機能開発
  * モデルのCRUD作成
  * Ajaxを使ったファイルアップロード・ダウンロード機能
  * 検索機能

#### ビジネスマッチングプラットフォーム: Linkers の保守
2017/11-現在

プラットフォーム利用者（社内外）から上がってきた不具合報告を調査して対応

* 【担当フェーズ】保守
* 企業の二重登録を避けるアルゴリズムの不備を調査して修正
* 日付に依存するテストを発見して修正
* etc.


### 2016/04(新卒入社) - 2017/10: 株式会社オロ
職務: ビジネスソリューション事業部 エンジニア

#### [社内案件] 経営戦略会議用資料作成の省力化
2017/03-2017/05

経営戦略会議用資料を作成するためのExcel VBAを実装

* 【担当フェーズ】要件定義、基本設計、詳細設計、プログラミング、導入、運用保守、テスト
  * 海外拠点で作成された現地通貨ベースの会計資料から日本円ベースの連結の会計資料に変換するマクロを作成
  * ヒアリングを通した要件定義から導入まで、プロジェクト管理をほぼ1人で担当
  * 新たな担当者に引き継ぎを行い、進捗管理と依頼者とのコミュニケーションを補助

#### ERPパッケージ: ZAC のカスタマイズ
2016/06-2017/10

Classic ASPで構成されたWebアプリケーションを、既存顧客向けにカスタマイズする

* 【担当フェーズ】詳細設計、プログラミング、単体テスト
  * 売上・販売管理システムのグループ間連携、DB間の連携をメインに設計及び実装
  * 特定の状況で稟議が自動承認されるロジックをDBサーバに実装
  * CSVファイルを出力するロジックと条件抽出のための画面を作成


## 趣味
* 合唱
  * [Gaia Philharmonic Choir \| Official Web Site](http://gaia2001.com/)
  * 上記の合唱団に参加しており、パートはテノール
  * 事務局長として、団体の運営に幹部としてかかわっている
  * カラオケも好きです

## 本経歴書の作成に参考にしたリンク
* [オープン職務経歴書を書いてみた \- Qiita](https://qiita.com/Sa2Knight/items/4af2f24fac9290d26119)
* [okohs/Curriculum\-Vitae\-template](https://github.com/okohs/Curriculum-Vitae-template)
  * リポジトリのフォーク元


以上